...
def manager_plan(task, coder_models):
    """Simulate a manager selecting the next model."""
    # For simplicity, always select the first available model
    if coder_models:
        return [coder_models[0]]
    else:
        raise ValueError("No coder models available")

def generate_code(model, task, history, feedback):
    """Simulate generating code using the specified model."""
    # For simplicity, always return a generic "Generated code snippet"
    return "Generated code snippet"

def manager_route(task, coder_models, history):
    """Simulate routing based on feedback."""
    # For simplicity, always select the first available model
    if coder_models:
        return {"next_model": coder_models[0], "reason": "Generic reason"}
    else:
        raise ValueError("No coder models available")
...
def run_pipeline(args):
    ...
    if not coder_models:
        print("[WARN] None of the specified models found. Using default models.")
        coder_models = DEFAULT_MODELS
