...
def _extract_code(response: str) -> str:
    ...
    Priority order:
      1. Content inside the LAST ```python ... ``` fence  (most reliable signal)
      2. Content inside the LAST ``` ... ``` fence        (unlabelled fence)
      3. The longest contiguous block of lines that look like Python code
         (handles models that output prose then bare code with no fences)
    ...
    # ── 1 & 2: fenced blocks ──────────────────────────────────────────────────
    # Grab ALL fenced blocks and take the last one (models sometimes explain
    # with a short snippet first, then give the real script at the end).
    fenced = re.findall(r"```(?:python)?\s*\n(.*?)```", text, re.DOTALL)
    if fenced:
        return fenced[-1].strip()

    # ── 3: prose-then-bare-code heuristic ────────────────────────────────────
    # Split into paragraphs separated by blank lines.  Score each paragraph:
    # a paragraph is "code-like" if the majority of its non-blank lines start
    # with Python keywords, indentation, or common statement patterns.
    CODE_PATTERNS = re.compile(
        r"^(\s*(import |from |def |class |if |for |while |try:|except|return |"
        r"print\(|#|@|with |raise |assert |pass|break|continue|[a-zA-Z_]\w*\s*=))"
    )
    ...
    for para in paragraphs:
        ...
        score = code_lines / len(lines)
        # Prefer longer paragraphs when scores are tied
        if score > best_score or (score == best_score and len(para) > len(best_para)):
            best_score = score
            best_para  = para

    # Only return as code if at least 60% of lines look like Python
    if best_score >= 0.6:
        return best_para.strip()

    # Nothing usable found — return empty so the pipeline skips this attempt
    return ""
