...
def _extract_code(response: str) -> str:
    ...
    Priority order:
      1. Content inside the LAST ```python ... ``` fence
      2. Content inside the LAST ``` ... ``` fence
      3. Response with leading prose stripped, scored line-by-line for Python
         (handles qwen-style "Here is the code:\n\nimport sys\n..." responses)
    ...
    # ── 3: prose-stripped heuristic ──────────────────────────────────────────
    # Identify lines that look like Python code
    ...
    # Identify lines that are clearly English prose (not code)
    PROSE_LINE = re.compile(
        r"^[A-Z][a-z].*\.$"          # Sentence starting with capital, ending with period
    ...
    # Find the first line that looks like code — discard everything before it
    code_start = 0
    for i, line in enumerate(lines):
        ...
        if CODE_LINE.match(line):
            code_start = i
            break
        # If it's clearly prose, keep scanning
        if PROSE_LINE.match(stripped):
            continue
        # Ambiguous non-empty line — stop discarding
        code_start = i
        break
    ...
    # Score the candidate block
    non_blank = [l for l in candidate_lines if l.strip()]
    if not non_blank:
        return ""

    code_lines = sum(1 for l in non_blank if CODE_LINE.match(l))
    score = code_lines / len(non_blank)

    # Lower threshold to 50% — we've already stripped leading prose above
    if score >= 0.5:
        return "\n".join(candidate_lines).strip()

    # Last resort: find the highest-scoring paragraph in the full text
    ...
    for para in paragraphs:
        ...
        if not para_lines:
            continue
        s = sum(1 for l in para_lines if CODE_LINE.match(l)) / len(para_lines)
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    while iteration < args.max_iterations:
        ...
        # Test — delegated entirely to test_runner module
